...
/**
 * Wraps a byte[] and provides useful methods to operate on it. Also provides
 * numerous factory methods to create instances based on various data types.
 * 
 * <p>
 * In an effort to provide a very stable and well tested marshalling strategy
 * this class uses the various methods available on {@link java.nio.ByteBuffer}
 * to perform serialization. The exceptions to this are the UUID and String
 * methods (see their javadoc comments for details).
 * </p>
 * 
 * <p>
 * The down side of the marshalling strategy used in this class this class is a
 * LOT of ByteBuffer instances being created. If you think this will be an issue
 * then consider avoiding the {@link #fromBoolean(boolean) various}
 * {@link #toBoolean() helper} {@link #fromTimeUuid(com.eaio.uuid.UUID) methods}
 * and instead use the {@link #fromBytes(byte[])} and {@link #getBytes()}
 * methods directly.
 * </p>
 */
public class Bytes {
	...
	/**
	 * Returns a string representation of the bytes as defined by the
	 * {@link java.util.Arrays#toString(byte[])} method.
	 * <p>
	 * <b>NOTE</b>: The {@link #toUTF8()} method provides the reverse value of
	 * the {@link #fromUTF8(String)} method.
	 * 
	...
	/**
	 * Determines if two instances of this class are equals using the
	 ...
	 * <p>
	 * Note that the instances hashCode is calculated the first time this method
	...
	/**
	 * Creates an instance based on the provided byte array. The major
	 * difference from the {@link Bytes#Bytes(byte[])} method is that this will
	...
	public static Bytes fromChar(char value) {
		return new Bytes(ByteBuffer.allocate(SIZEOF_CHAR).putChar(value)
				.array());
	}
	...
	public static Bytes fromLong(long value) {
		return new Bytes(ByteBuffer.allocate(SIZEOF_LONG).putLong(value)
				.array());
	}
	...
	public static Bytes fromShort(short value) {
		return new Bytes(ByteBuffer.allocate(SIZEOF_SHORT).putShort(value)
				.array());
	}
	...
	public static Bytes fromDouble(double value) {
		return new Bytes(ByteBuffer.allocate(SIZEOF_DOUBLE).putDouble(value)
				.array());
	}
	 ...
	 */
	public static Bytes fromDouble(Double value) {
		return value == null ? new Bytes(null)
				: fromDouble(value.doubleValue());
	}
	...
	public static Bytes fromFloat(float value) {
		return new Bytes(ByteBuffer.allocate(SIZEOF_FLOAT).putFloat(value)
				.array());
	}
	...
	public static Bytes fromBoolean(boolean value) {
		return new Bytes(ByteBuffer.allocate(SIZEOF_BOOLEAN)
				.put(value ? BOOLEAN_TRUE : BOOLEAN_FALSE).array());
	}
	 ...
	 */
	public static Bytes fromBoolean(Boolean value) {
		return value == null ? new Bytes(null) : fromBoolean(value
				.booleanValue());
	}

	/**
	 * Creates an instance based on the provided value handling nulls. If the
	 * provided value is not null then this method delegates to
	 * {@link #fromUuid(long, long)} to perform the deserialization.
	 * 
	 * @param value
	 *            the value
	 * @return the instance or null if the value provided was null
	...
	public static Bytes fromUuid(UUID value) {
		return value == null ? new Bytes(null)
				: fromUuid(value.getMostSignificantBits(),
						value.getLeastSignificantBits());
	}

	/**
	 * Creates an instance based on the provided value handling nulls. If the
	 * provided value is not null then a new instance of {@link UUID} is created
	 * using {@link java.util.UUID#fromString(String)} and then
	...
	public static Bytes fromUuid(String value) {
		return value == null ? new Bytes(null) : fromUuid(UUID
				.fromString(value));
	}

	/**
	 * Creates an instance based on the provided values. The
	 * {@link java.nio.ByteBuffer#putLong(long)} is called twice, the first call
	 * for the msb value and second for the lsb value.
	 * 
	 * @param msb
	 *            the msb value
	 * @param lsb
	...
	public static Bytes fromUuid(long msb, long lsb) {
		return new Bytes(ByteBuffer.allocate(SIZEOF_UUID).putLong(msb)
				.putLong(lsb).array());
	}
	 ...
	 * Creates an instance based on the provided value handling nulls. If the
	 * provided value is not null then this method delegates to
	 * {@link #fromTimeUuid(long, long)} to perform the deserialization.
	 * 
	 * @param value
	 *            the value
	 * @return the instance or null if the value provided was null
	...
	public static Bytes fromTimeUuid(com.eaio.uuid.UUID value) {
		return value == null ? new Bytes(null) : fromTimeUuid(value.getTime(),
				value.getClockSeqAndNode());
	}

	/**
	 * Creates an instance based on the provided value handling nulls. If the
	 * provided value is not null then a new instance of
	 * {@link com.eaio.uuid.UUID} is created and then
	 * {@link #fromTimeUuid(long, long)} is called to perform the
	 * deserialization.
	 * 
	 * @param value
	 *            the value
	...
	public static Bytes fromTimeUuid(String value) {
		return value == null ? new Bytes(null)
				: fromTimeUuid(new com.eaio.uuid.UUID(value));
	}

	/**
	 * Creates an instance based on the provided values. The
	 * {@link java.nio.ByteBuffer#putLong(long)} is called twice, the first call
	 * for the time value and second for the clockSeqAndNode value.
	 * 
	 * @param time
	 *            the time value
	...
	public static Bytes fromTimeUuid(long time, long clockSeqAndNode) {
		return new Bytes(ByteBuffer.allocate(SIZEOF_UUID).putLong(time)
				.putLong(clockSeqAndNode).array());
	}

	/**
	 * Creates an instance based on the provided value in UTF-8 format handling
	 * nulls.
	 * 
	 * @param value
	 *            the value
	 ...
	 * 
	 * @param defaultIfNull
	 *            the value to return is the backing array is null or empty
	 ...
	 *             if the underlying array does not contain the appropriate data
	 */
	public char toChar() throws IllegalStateException {
		try {
			...
			throw new IllegalStateException(
					"Failed to read value due to invalid format.  See cause for details...",
					e);
		}
	}
	...
	/**
	 * Transforms the provided list of {@link Bytes} instances into a list of
	 * byte arrays.
	 * 
	 * @param arrays
	...
	/**
	 * Transforms the provided list of {@link String} instances into a list of
	 * byte arrays.
	 * 
	 * @param strings
	 *            the list of Bytes instances
	...
	/**
	 * Returns the underlying byte array of the provided bytes instance or null
	 * if the provided instance was null.
	 * 
	 * @param bytes
	 *            the bytes instance
